...
app.get("/showTodo", (req, res) => {
    ...
    User.findOne({
        ...
        if (!user) {
            ...
            arr = user.todo
            table = "<table style='width:100%; padding:100px;' class='table table-striped'><tr><th>Description</th><th>Completed</th></tr>";
            for (var i = 0; i < arr.length; i++) {
                table += "<tr><td>" + arr[i] + "</td><td><form action='/deleteTodo?token=" + req.query.token + "&email=" + req.query.email + "&todo=" + arr[i] + "' method='post'><input class='btn btn-danger' type='submit' value='Incomplete'></form></td></tr>"
            }
            table += "</table>"
